...
export default function Cursor(props: {
  ...
  form: "default" | "mousedown" | "up" | "down";
  cursorVisible: boolean;
  setForm: any;
}) {
  const { currentDirection, form, setForm, cursorVisible } = props;
  ...
  const setPoint = (info: any) => {
    ...
    if (xTo.current && yTo.current) {
      ...
      yTo.current(mouseInfoRef.current.y - cursorRefRect.current.height / 2);
    }
  };
  useEffect(() => {
  ...
  return (
    <div
      ref={cursorRef}
      className={clsx(
        "absolute w-24 h-24 z-[calc(var(--maxZIndex)_+_1)] flex justify-center items-center text-white select-none pointer-events-none opacity-0 origin-[0%_0%]"
      )}
    >
      <div
        className={`${
