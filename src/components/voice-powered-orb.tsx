...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      let lastTimeForScroll = performance.now();
      const scrollControl = (e: any) => {
        const now = performance.now();
        const dy = e.deltaY;
        const dt = now - lastTimeForScroll;

        const speed = dy / dt;
        scrollSpeed.current = Math.abs(speed);
        lastTimeForScroll = now;
      };

      function scrollVelocityTick() {
        ...
        requestAnimationFrame(scrollVelocityTick);
      }
      scrollVelocityTick();
